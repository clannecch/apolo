...
#ojo 29-08-2011   has_many                :detalle_recibos
  ...
  after_validation         :validar_para_recibo, :on => :update, :if => :employee_id?
...
# "self.cantidad * acumuladores.basico"
  ...
  def validar_para_recibo
     detalle_recibo_habers.each do |rh|
       ['cantidad', 'importe'].each do |campo|
         errors.add(campo, ">>>>Debe Indicar #{campo.capitalize} en Haber") if rh.remunerative_concept.data_to_ask.send("#{campo}?") && rh.send(campo).to_i.zero?
         errors.add(campo, ">>>>>No debe indicar #{campo.capitalize} en Haber" ) if !rh.remunerative_concept.data_to_ask.send("#{campo}?") && rh.send(campo).present?
       end
     end
    return !errors.any?
  end
...
#      self.fecha_calculo = DateTime.now
      ...
      errors.add(:base, "Problemas al calcular al legajo="+employee.legajo)
    end
  end
  ...
  def carga_codigo_predefinido_create
      ...
      detalle_recibo_haber_adicionados = self.employee.group_remuneration.remunerative_concepts.map do |rc|
        detalle_recibo_habers.build(:remunerative_concept_id => rc.id,
                                    :cost_center_id => employee.cost_center_id)
      end

      self.employee.group_retention.retention_concepts.each do |rc|
        detalle_recibo_retencions.build(:retention_concept_id => rc.id,
                                        :cost_center_id => employee.cost_center_id)
      end

      detalle_recibo_haber_adicionados.each do |rc|
        if rc.remunerative_concept.concepto_asociado_haber_id.present?
          detalle_recibo_habers.build(:remunerative_concept_id => rc.remunerative_concept.concepto_asociado_haber_id,
                                      :cost_center_id => employee.cost_center_id)
        end
...
# si existe en employee ese metodo lo retorna, caso contrario continua con el default del method_missing
  ...
  def controlar_cambios
    @recalcular =  false
    if !@recalcular
      self.detalle_recibo_habers.each do |rc|
        if rc.new_record?
          @recalcular = true
        else
          if   rc.cantidad != rc.cantidad_was                               ||
               rc.importe != rc.importe_was                                 ||
               rc.remunerative_concept_id != rc.remunerative_concept_id_was
            @recalcular = true
          end
        end
      end
    ...
    if !@recalcular
      self.detalle_recibo_retencions.each do |rc|
        if rc.new_record?
          @recalcular = true
        else
          if rc.cantidad != rc.cantidad_was                               ||
               rc.importe != rc.importe_was                               ||
               rc.retention_concept_id != rc.retention_concept_id_was
            @recalcular = true
          end
        end
      end
